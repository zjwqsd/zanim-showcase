"""Lesson 13: Scene-owned transforms on a mathematically unbounded plane.
...
def _apply_together(
    scene: Scene, objects, provider: LinearMap, *, duration: float
) -> None:
...
class InfiniteSpace(Scene):
    def setup(self) -> None:
        ...
        self.x_axis = InfiniteLine(
            (0, 0), (1, 0), color=RED.with_alpha(220), stroke_width=0.035, z_index=-2
        )
        self.y_axis = InfiniteLine(
            (0, 0), (0, 1), color=GREEN.with_alpha(220), stroke_width=0.035, z_index=-2
        )
        self.reference = Polygon(
            [
                (0.45, 0.35),
                (2.05, 0.35),
                (2.05, 0.85),
    ...
    def construct(self) -> None:
        ...
        _stage(
            ...
            lambda a: Transform2D(xx=1.0, xy=0.65 * a, yx=0.0, yy=1.0 - a),
            duration=1.9,
            hold=0.62,
        )
        _stage(
