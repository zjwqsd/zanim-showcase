...
def link(length: float, color: Color):
...
class Kinematics(Scene):
    def construct(self) -> None:
        ...
        joint3 = Group(
            [
                Dot(ORIGIN, radius=0.11, color=ORANGE, z_index=4),
                link(l3, ORANGE),
                Dot(Vec2(l3, 0), radius=0.13, color=Color(255, 224, 105), z_index=5),
            ],
            position=(l2, 0),
        )
        joint2 = Group(
            [Dot(ORIGIN, radius=0.11, color=WHITE, z_index=4), link(l2, GREEN), joint3],
            position=(l1, 0),
        )
        joint1 = Group(
            [Dot(ORIGIN, radius=0.11, color=WHITE, z_index=4), link(l1, BLUE), joint2]
        )

        title = Text("Open-chain FK = ordinary frame composition", font_size=32)
        formula = Text("T₀ₑ = T₀₁(q₁) · T₁₂(q₂) · T₂₃(q₃)", font_size=25, color=MUTED)
        title.place(anchor=TOP, at=self.frame.top + Vec2(0, -0.35))
        formula.place(anchor=TOP, at=title.anchor(BOTTOM) + Vec2(0, -0.16))
        joint1.place(anchor=CENTER, at=Vec2(-0.6, -0.55))

        home1 = SE2.from_affine(joint1.transform)
        home2 = SE2.from_affine(joint2.transform)
        home3 = SE2.from_affine(joint3.transform)

        self.add(title, formula)
        joint1 = self.add(joint1)
        joint2 = joint1.children[-1]
        joint3 = joint2.children[-1]
        self.wait(0.6)
